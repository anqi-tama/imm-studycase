...
import { RSPAD_DATA } from './data/rspad.ts';
...
const App: React.FC = () => {
  ...
  const getCaseData = (id: CaseStudy): CaseStudyData => {
    switch(id) {
      case 'wardah': return WARDAH_DATA;
  ...
  const currentData = getCaseData(currentCase);
  ...
  const getThemeColor = () => {
    if (currentCase === 'wardah') return 'teal';
    if (currentCase === 'ugm') return 'indigo';
    return 'emerald';
  };
            ...
            <div className="flex items-center space-x-8">
              ...
              <div className="flex items-center space-x-4">
                <button 
                  ...
                  className={`flex items-center text-xs font-bold h-16 px-1 border-b-2 transition-all ${currentCase === 'wardah' ? 'text-teal-700 border-teal-600' : 'text-gray-500 border-transparent hover:text-teal-600'}`}
                >
                  Study Case Wardah
                </button>
                <button 
                  onClick={() => setCurrentCase('ugm')}
                  className={`flex items-center text-xs font-bold h-16 px-1 border-b-2 transition-all ${currentCase === 'ugm' ? 'text-indigo-700 border-indigo-600' : 'text-gray-500 border-transparent hover:text-indigo-600'}`}
                >
                  Study Case UGM
                </button>
                <button 
                  onClick={() => setCurrentCase('rspad')}
                  className={`flex items-center text-xs font-bold h-16 px-1 border-b-2 transition-all ${currentCase === 'rspad' ? 'text-emerald-700 border-emerald-600' : 'text-gray-500 border-transparent hover:text-emerald-600'}`}
                >
                  Study Case RSPAD
                </button>
              </div>
          ...
          <div>
            ...
            <div className="flex items-center mt-2 text-gray-500 font-medium">
              <span className={`px-3 py-1 rounded-full text-xs font-bold mr-3 ${
                currentCase === 'wardah' ? 'bg-teal-50 text-teal-700' : 
                currentCase === 'ugm' ? 'bg-indigo-50 text-indigo-700' : 
        ...
        <div className="space-y-16 print:space-y-0">
          <section id="summary" className="scroll-mt-20">
            <div className={`mb-6 border-b-2 pb-2 flex items-center justify-between ${
              currentCase === 'wardah' ? 'border-teal-600' : 
              currentCase === 'ugm' ? 'border-indigo-600' : 
              'border-emerald-600'} print:mb-3`}>
               <h3 className={`text-2xl font-bold ${
                 currentCase === 'wardah' ? 'text-teal-800' : 
                 currentCase === 'ugm' ? 'text-indigo-800' : 
                 'text-emerald-800'}`}>
            ...
            {[
              ...
              <button
                ...
                className={`flex-1 flex flex-col items-center justify-center py-2 px-1 rounded-xl transition-all ${
                  activeTab === tab.id 
                  ? (currentCase === 'wardah' ? 'bg-teal-600 shadow-teal-100' : 
                     currentCase === 'ugm' ? 'bg-indigo-600 shadow-indigo-100' : 
                     'bg-emerald-600 shadow-emerald-100') + ' text-white shadow-lg' 
